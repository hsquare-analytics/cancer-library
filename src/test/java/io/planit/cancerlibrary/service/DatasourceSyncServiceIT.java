...
class DatasourceSyncServiceIT {
    ...
    void test_sync_fdx_update_not_exist() {
        // given
        ...
        // when
        datasourceSyncService.syncPatientFdx(patient.detail(detail), "test");
        ...
        result.forEach(map -> assertThat(map).containsEntry("fdx", "20190101"));
    }

    @Test
    @Sql(scripts = "classpath:sql/cncr_rgst.ddl.sql")
    void test_sync_fdx_update_exist() {
        // given
        Patient patient = PatientResourceIT.createPatientDTO();
        PatientDetail detail = patient.getDetail().standardDate(Instant.parse("2019-01-01T00:00:00Z"));
    ...
    @Test
    @Sql(scripts = "classpath:sql/cncr_rgst.ddl.sql")
    void test_sync_fdx_of_only_updated_exist() {
        // given
        Patient patient = PatientResourceIT.createPatientDTO();
        PatientDetail detail = patient.getDetail().standardDate(Instant.parse("2019-01-01T00:00:00Z"));
        ...
        // when
        datasourceSyncService.syncOnlyUpdatedExistPatientFdx(patient.detail(detail), "test");
