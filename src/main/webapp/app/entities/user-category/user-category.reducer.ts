...
import { IUserCategory, defaultValue } from 'app/shared/model/user-category.model';
...
const apiUrl = 'api/users-categories';

// Actions

export const getEntities = createAsyncThunk('user_category/fetch_entity_list', async ({ page, size, sort }: IQueryParams) => {
  const requestUrl = `${apiUrl}?cacheBuster=${new Date().getTime()}`;
  return axios.get<IUserCategory[]>(requestUrl);
});

export const getEntity = createAsyncThunk(
  'user_category/fetch_entity',
...
export const partialUpdateEntity = createAsyncThunk(
  'user_category/partial_update_entity',
  async (entity: IUserCategory, thunkAPI) => {
